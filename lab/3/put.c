...
int main(int argc,char* argv[]){
    ...
    //创建共享内存组
    share* rd = (share*)shmat(CreateShmId(419), NULL, 0);
    ...
    //ReadPid
    int rdfl = open("1",O_RDONLY);
    char Read[LENGTH];
    while(read(rdfl,Read,LENGTH) != 0){
        P(semid, 0);
        strcpy(rd->circleBuffer[rd->pin],Read);
        memset(Read,0,LENGTH);
        rd->pin++;
        rd->pin = rd->pin % CIRCLE;//循环队列
        V(semid, 1);
    }
    rd->pin--;
    rd->pend = rd->pin;
